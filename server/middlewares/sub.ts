...
import {curTimeWithinSubRange} from "../utils/subscription/sub_helpers";
...
export async function addSubMiddleware(req: Request, res: Response, next: NextFunction) {
  ...
  if (!curTimeWithinSubRange(subscription.subStart, subscription.subEnd))
    return res.status(400).json({msg: `Current subscription is not yet begun`})
