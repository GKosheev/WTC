...
export async function forgotPasswordMessage(name: string, email: string, token: string) {
  ...
  const text: string =
    `Hello ${name}, \n\n
    Please reset your password by clicking the link: ${config.url}/reset-password/${token} \n\n
    Thank you! \n`
...
export async function confirmEmailMessage(name: string, email: string, token: string) {
  ...
  const text: string =
    `Hello ${name}, \n\n
   Please verify your account by clicking the link: ${config.url}/confirm-email/${token} \n\n
    Thank you!\n`
  await sendMessage(email, subject, text)
}
