...
import {CourtBooking} from '../interfaces/old interfaces/CourtBooking'
...
export async function postCourt(req: Request, res: Response) {
  ...
  const users: { members: OldUser[], guests: OldUser[] } = res.locals.players
  const creator = <OldUser>req.user
